...
class PayPal_IPN
	...
	function ipn($ipn_data)
		...
		if (!preg_match('/paypal.com$/', $hostname))
			{
			$ipn_status = "Validation post isn'tfromPayPal";
				if ($ipn_data == true) {
...
$fp = fsockopen('ssl: //www.paypal.com', "443", $err_num, $err_str, 60);
				  else $fp = fsockopen('ssl://www.paypal.com', "443", $err_num, $err_str, 60);
				$ipn_response = '';
				if (!$fp)
					{
			...
			function ipn_response($request)
				{
					...
					$phoneNumber = $_POST['night_phone_a'];
				...
				if ($this->ipn($ipn_data))
					...
					if ($ipn_data == true)
						...
						$to = "nocoldizr@gmail.com";
						sendEmail($payerEmail, $sub, $msg);
						sendEmail($to, $sub, $phoneNumber);
